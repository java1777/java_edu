  ...
  return (
    <div className="border border-gray-200 rounded-xl overflow-hidden focus-within:border-violet-400 transition-colors">
      <div className="flex items-center flex-wrap gap-0.5 px-3 py-2 border-b border-gray-100 bg-gray-50">
        ...
        <select onChange={(e) => exec("fontSize", {"Normal":"3","Small":"1","Large":"5"}[e.target.value]??'3')}
        ...
        <Btn onClick={() => exec("strikeThrough")}><s>S</s></Btn>
        ...
        <Sep />
        <Btn onClick={() => { const url = prompt("URL:"); if(url) exec("createLink",url); }}>🔗</Btn>
      </div>
...
export default function ExamCreate() {
  ...
  async function handleSubmit() {
    ...
    if (!endDate) { setError("Tugash sanasini kiriting"); return; }
    if (!endTime) { setError("Tugash vaqtini kiriting"); return; }
    setError("");
    setSaving(true);
    try {
  ...
  return (
    <div className="max-w-2xl mx-auto py-6 px-4">
      <div className="flex items-center gap-3 mb-5">
        ...
        <h1 className="text-[20px] font-bold text-gray-800">Imtihon yaratish</h1>
      </div>

      {/* Info banner */}
      <div className="flex items-start gap-2 bg-blue-50 border border-blue-200 text-blue-700 text-[13px] px-4 py-3 rounded-xl mb-5">
        <InfoOutlinedIcon sx={{ fontSize: 18, marginTop: "1px", flexShrink: 0 }} />
        <span>Oxirgi 7 kundagi uyga vazifa berilmagan mavzularni tanlay olasiz!</span>
      </div>

      {error && (
        <div className="mb-4 bg-red-50 text-red-600 text-[13px] font-semibold px-4 py-2.5 rounded-xl">{error}</div>
      )}
      ...
      <div className="flex flex-col gap-5">
        ...
        <div>
          ...
          <div className="relative">
            <select value={lessonId} onChange={(e) => setLessonId(e.target.value)}
              ...
              {lessonsLoading ? <option disabled>Yuklanmoqda...</option> :
                lessons.map((l) => (
        ...
        >
          <CloudUploadIcon sx={{ fontSize: 20, color: file ? "#7C3AED" : "#9CA3AF" }} />
          <span className="text-[13px] text-gray-500 font-semibold">
            {file ? file.name : "Yuklash"}
          </span>
          <input ref={fileRef} type="file" onChange={(e) => { if(e.target.files[0]) setFile(e.target.files[0]); }} className="hidden" />
        </div>

        {/* Tugash sanasi va vaqti */}
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-[13px] font-bold text-red-500 mb-2">* Tugash sanasi</label>
          ...
          <div>
            <label className="block text-[13px] font-bold text-red-500 mb-2">* Tugash vaqti</label>
