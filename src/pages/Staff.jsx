...
const AVATAR_COLORS = [
  "#7C3AED", "#2563EB", "#EA580C", "#16A34A", "#DB2777", "#0891B2", "#9333EA", "#CA8A04",
];

const EMPTY = { first_name: "", last_name: "", phone: "", email: "", address: "", password: "" };
...
export default function Staff() {
  ...
  return (
    <>
      {/* Tab navigation */}
      <div className="mb-6">
        <h1 className="text-2xl font-extrabold text-gray-800 mb-4">Boshqarish</h1>
      ...
      {apiError && (
        <div className="bg-red-50 text-red-600 text-[13px] px-4 py-3 rounded-xl mb-4">{apiError}</div>
      )}

      {/* Staff grid */}
      {loading ? (
        <div className="text-center text-[13px] text-gray-400 py-16">Yuklanmoqda...</div>
      ) : staff.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 text-gray-400">
          <div className="w-16 h-16 rounded-2xl bg-gray-100 flex items-center justify-center mb-3">
            <PersonAddIcon sx={{ fontSize: 28, color: "#D1D5DB" }} />
          </div>
          <p className="text-[14px] font-semibold text-gray-500">Hodimlar yo'q</p>
          <p className="text-[12px] text-gray-400 mt-1">Yangi hodim qo'shish uchun tugmani bosing</p>
        </div>
      ) : (
                ...
                <div className="px-5 py-3 flex flex-col gap-2">
                  {s.phone && (
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[12px] text-gray-600 truncate">{s.phone}</span>
                    </div>
                  )}
                  {s.email && (
                    <div className="flex items-center gap-2">
                      <EmailIcon sx={{ fontSize: 14, color: "#9CA3AF" }} />
                      <span className="text-[12px] text-gray-600 truncate">{s.email}</span>
                    </div>
                  )}
                  {s.address && (
                    <div className="flex items-center gap-2">
                      <LocationOnIcon sx={{ fontSize: 14, color: "#9CA3AF" }} />
                      <span className="text-[12px] text-gray-600 truncate">{s.address}</span>
                    </div>
                  )}
                </div>
              </div>
      ...
      {deleteId && (
        <div className="fixed inset-0 z-60 flex items-center justify-center">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl px-8 py-7 w-80 z-10">
            <h3 className="text-[17px] font-bold text-gray-800 mb-2">Hodimni o'chirish</h3>
      ...
      {/* Add drawer backdrop */}
      <div onClick={closeDrawer} className="fixed inset-0 z-40 transition-all duration-500"
        style={{ background: drawerOpen ? "rgba(0,0,0,0.35)" : "transparent", pointerEvents: drawerOpen ? "auto" : "none" }} />

      {/* Add drawer */}
      <div className="fixed top-0 right-0 h-full w-full sm:w-96 bg-white shadow-2xl z-50 flex flex-col transition-transform duration-500 ease-in-out"
        style={{ transform: drawerOpen ? "translateX(0)" : "translateX(100%)" }}>
        <div className="px-6 pt-6 pb-4 border-b border-gray-100 flex items-center justify-between">
          <span className="text-[16px] font-bold text-gray-800">Yangi hodim qo'shish</span>
          <button onClick={closeDrawer} className="w-7 h-7 flex items-center justify-center rounded-lg hover:bg-gray-100 cursor-pointer">
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 flex flex-col gap-4">
          {errors.api && (
            <p className="text-[12px] text-red-500 bg-red-50 rounded-xl px-3 py-2">{errors.api}</p>
          )}
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="block text-[13px] font-semibold text-gray-700 mb-1.5">Familya <span className="text-red-500">*</span></label>
              <input placeholder="Familya" value={form.last_name}
          ...
          <div>
            <label className="block text-[13px] font-semibold text-gray-700 mb-1.5">
              <PhoneIcon sx={{ fontSize: 14 }} /> Telefon <span className="text-red-500">*</span>
            </label>
            <input type="tel" placeholder="+998XXXXXXXXX" value={form.phone}
          ...
          <div>
            ...
            <input placeholder="Shahar, ko'cha..." value={form.address}
              onChange={(e) => setForm((p) => ({ ...p, address: e.target.value }))}
              className={inp(false)} />
          </div>

          <div>
            <label className="block text-[13px] font-semibold text-gray-700 mb-1.5">Parol <span className="text-red-500">*</span></label>
            <input type="password" placeholder="Parolni kiriting" value={form.password}
